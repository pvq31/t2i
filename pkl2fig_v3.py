...
from inference.read_pkl import export_pkl_contents, save_exported_json
...
def cube_volume_from_subject(subject_dict: Dict[str, Any]) -> float:
    dims = subject_dict.get("dims", (0.0, 0.0, 0.0))
    if not isinstance(dims, (list, tuple)) or len(dims) != 3:
        return 0.0
    return float(dims[0]) * float(dims[1]) * float(dims[2])


def print_cube_volumes(raw_subjects: List[Dict[str, Any]]) -> None:
    print("[Info] cube 尺寸和体积:")
    for subject_index, subject_dict in enumerate(raw_subjects):
        dims = subject_dict.get("dims", (0.0, 0.0, 0.0))
        if not isinstance(dims, (list, tuple)) or len(dims) != 3:
            print(f"  {subject_index + 1}. {subject_dict.get('name', f'cuboid_{subject_index}')}: dims 非法 {dims}")
            continue
        volume = cube_volume_from_subject(subject_dict)
        print(
            f"  {subject_index + 1}. {subject_dict.get('name', f'cuboid_{subject_index}')}: "
            f"dims=({float(dims[0]):.6g}, {float(dims[1]):.6g}, {float(dims[2]):.6g}), "
            f"volume={volume:.9g}"
        )
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        nargs="?",
        default=None,
        help="位置参数写法的场景 pkl 路径，例如 inference/saved_scenes/test4.6/example_cube_test4.6_shiyan1_v1.pkl",
    )
    parser.add_argument(
        "output_positional",
        nargs="?",
    ...
    parser.add_argument(
        ...
        dest="scene_pkl",
        default=None,
        help="场景 pkl 路径，例如 inference/saved_scenes/test4.6/example_cube_test4.6_shiyan1_v1.pkl",
    )
    parser.add_argument(
        "-output",
        "--output",
    ...
    args.scene_pkl = (
        ...
        or "inference/saved_scenes/test4.6/example_cube_test4.6_2_fixed.pkl"
    )
...
def build_cube_layout_filename(scene_pkl_path: str) -> str:
    ...
    elif pkl_basename.startswith("example_cube_"):
        cube_basename = f"cube_{pkl_basename[len('example_cube_'):] }"
    else:
...
def resolve_json_output_path(scene_pkl_path: str) -> str:
    return os.path.splitext(scene_pkl_path)[0] + ".json"


def export_scene_json(scene_pkl_path: str) -> str:
    output_json_path = resolve_json_output_path(scene_pkl_path)
    exported_scene = export_pkl_contents(scene_pkl_path)
...
def render_cube_image_pil(
    ...
    cubes_layer = Image.new("RGBA", image.size, (0, 0, 0, 0))
    for _, polygon, color in sorted(faces, key=lambda item: item[0], reverse=True):
        scaled = scaled_polygon(polygon)
        cubes_draw = ImageDraw.Draw(cubes_layer, "RGBA")
        cubes_draw.polygon(scaled, fill=(*color, cube_face_alpha))
        cubes_draw.line(
            scaled + [scaled[0]],
    ...
    if bbox is not None:
        ...
        shift_x = round(target_center - component_center_x)
        shift_y = round(target_center - component_center_y)
        centered_layer = Image.new("RGBA", image.size, (0, 0, 0, 0))
        centered_layer.alpha_composite(cubes_layer, dest=(shift_x, shift_y))
        cubes_layer = centered_layer

    image = Image.alpha_composite(image, cubes_layer)
...
def add_plane():
    ...
    material = make_opaque_material('GroundWhiteMaterial', (1.0, 1.0, 1.0, 1.0), 0.7)
...
def main() -> None:
    ...
    output_json_path = export_scene_json(scene_pkl_path)
    ...
    try:
        ...
        print(f"[Done] scene JSON 已保存到: {output_json_path}")
        print(f"[Info] 物体数量: {len(subjects_data)}")
        print(f"[Info] 渲染模式: {args.render_mode}")
        print_cube_volumes(scene_dict.get("subjects_data", []))
