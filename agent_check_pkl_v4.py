...
GUIDANCE_WIDER_TOKENS = ("wider", "widen", "broader", "更宽", "加宽", "变宽")
GUIDANCE_NARROWER_TOKENS = ("narrower", "less wide", "更窄", "变窄", "缩窄")
GUIDANCE_DEEPER_TOKENS = ("deeper", "increase depth", "more depth", "更深", "加深")
GUIDANCE_SHALLOWER_TOKENS = ("shallower", "reduce depth", "less deep", "更浅", "变浅")
GUIDANCE_TALLER_TOKENS = ("taller", "higher", "more tall", "更高", "加高", "变高")
GUIDANCE_SHORTER_TOKENS = ("shorter", "lower", "less tall", "更矮", "变矮", "降低高度")
...
def apply_guidance_directives(
    ...
    for subject in subjects:
        ...
        for clause in subject_relevant_clauses(guidance_text, subject):
            ...
            if clause_contains_any(clause, GUIDANCE_WIDER_TOKENS):
                subject["dims"][0] = round(max(subject["dims"][0], baseline_dims[0] * 1.18), 6)
            if clause_contains_any(clause, GUIDANCE_NARROWER_TOKENS):
                subject["dims"][0] = round(min(subject["dims"][0], baseline_dims[0] * 0.86), 6)
            if clause_contains_any(clause, GUIDANCE_DEEPER_TOKENS):
                subject["dims"][1] = round(max(subject["dims"][1], baseline_dims[1] * 1.16), 6)
            if clause_contains_any(clause, GUIDANCE_SHALLOWER_TOKENS):
                subject["dims"][1] = round(min(subject["dims"][1], baseline_dims[1] * 0.86), 6)
            if clause_contains_any(clause, GUIDANCE_TALLER_TOKENS):
                subject["dims"][2] = round(max(subject["dims"][2], baseline_dims[2] * 1.14), 6)
            if clause_contains_any(clause, GUIDANCE_SHORTER_TOKENS):
...
def main() -> None:
    ...
    guidance_pending_issues = collect_guidance_application_issues(
        current_subjects,
        current_subjects,
    ...
    for attempt in range(1, max_repair_attempts + 1):
        ...
        candidate_guidance_issues = collect_guidance_application_issues(
            current_subjects,
            candidate_subjects_summary,
            guidance_preview_subjects,
            guidance_target_names,
    ...
    if final_scene_dict is None or final_fit_result is None:
        ...
        print("警告：在设定的修正轮数内仍未达到严格完全合格，已输出当前最佳修正版。")
