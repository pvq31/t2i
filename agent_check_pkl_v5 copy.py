    ...
    for clause in split_text_clauses(guidance_text):
        ...
        if targets_global_scale:
            explicit_scale = extract_guidance_numeric_value(
                clause,
                ("global scale", "global_scale", "scale all cubes", "scale all cube", "scale all objects"),
            )
            if explicit_scale is not None and explicit_scale > 0.0:
                updated_camera["global_scale"] = round(clamp(explicit_scale, 0.05, 10.0), 6)
            elif any(token in normalized_clause for token in ("scale up", "larger", "increase", "放大", "增大")):
                updated_camera["global_scale"] = round(clamp(updated_camera["global_scale"] * 1.1, 0.05, 10.0), 6)
            elif any(token in normalized_clause for token in ("scale down", "smaller", "decrease", "reduce", "缩小", "减小")):
                updated_camera["global_scale"] = round(clamp(updated_camera["global_scale"] * 0.9, 0.05, 10.0), 6)

        targets_elevation = any(
            token in normalized_clause
            for token in ("camera elevation", "camera angle", "higher camera", "lower camera", "higher view", "lower view", "俯仰", "仰角", "相机")
        )
        if targets_elevation:
            explicit_elevation = extract_guidance_numeric_value(
                clause,
                ("camera elevation", "camera angle", "elevation", "camera_elevation"),
            )
            if explicit_elevation is not None:
                updated_camera["camera_elevation_deg"] = round(clamp(explicit_elevation, 0.0, 90.0), 6)
            elif any(token in normalized_clause for token in ("higher", "increase", "raise", "more top", "更高", "增大", "提高")):
                updated_camera["camera_elevation_deg"] = round(clamp(updated_camera["camera_elevation_deg"] + 2.0, 0.0, 90.0), 6)
            elif any(token in normalized_clause for token in ("lower", "decrease", "reduce", "更低", "减小", "降低")):
                updated_camera["camera_elevation_deg"] = round(clamp(updated_camera["camera_elevation_deg"] - 2.0, 0.0, 90.0), 6)

        targets_lens = any(
            token in normalized_clause
            for token in ("camera lens", "lens", "zoom", "wider camera view", "wider view", "视野", "焦距")
        )
        if targets_lens:
            explicit_lens = extract_guidance_numeric_value(clause, ("camera lens", "lens", "camera_lens"))
            if explicit_lens is not None:
                updated_camera["lens_mm"] = round(clamp(explicit_lens, MIN_CAMERA_LENS_MM, 200.0), 6)
            elif any(token in normalized_clause for token in ("wider view", "zoom out", "decrease", "reduce", "更广", "拉远")):
                updated_camera["lens_mm"] = round(clamp(updated_camera["lens_mm"] - 5.0, MIN_CAMERA_LENS_MM, 200.0), 6)
            elif any(token in normalized_clause for token in ("zoom in", "increase", "larger lens", "拉近")):
                updated_camera["lens_mm"] = round(clamp(updated_camera["lens_mm"] + 5.0, MIN_CAMERA_LENS_MM, 200.0), 6)
