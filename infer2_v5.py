...
V4_OBJECT_ALIASES = {
    ...
    "bookshelf": ["bookshelf", "wooden bookshelf", "bookcase", "shelf", "book shelf"],
    "chair": ["chair", "armchair", "office chair"],
    "table": ["table", "desk", "coffee table"],
    "bed": ["bed"],
    "cabinet": ["cabinet", "drawer cabinet", "cupboard"],
    "refrigerator": ["refrigerator", "fridge", "double-door refrigerator", "double door refrigerator"],
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="加载一个或多个场景 pkl 进行推理，并通过命令行传入 scene pkl 和 prompt。",
        epilog=(
            '示例:\n'
            '  python infer20.py '
            ...
            '  python infer20.py '
            '--scene-pkls inference/saved_scenes/example_test3.31_1_fixed.pkl '
            '--placeholder-prompt "In the image, there is a refrigerator in the center."'
        ),
    ...
    parser.add_argument(
        "--placeholder-prompt",
        required=True,
        help="送入 FLUX 的 prompt。可包含 PLACEHOLDER；也可直接写完整自然语言 prompt。",
    )
    parser.add_argument(
        "--image-size",
        type=int,
...
def copy_scene_sidecar_files(pkl_path: str, output_dir: str) -> List[str]:
    """Copy the scene pkl and same-name .json/.jpg files into the run folder."""
    scene_dir = os.path.dirname(os.path.abspath(pkl_path))
    scene_stem = os.path.splitext(os.path.basename(pkl_path))[0]
    copied_paths = []
    for extension in (".pkl", ".json", ".jpg"):
        source_path = os.path.join(scene_dir, f"{scene_stem}{extension}")
...
def build_filled_prompt(subject_descriptions: List[str], prompt: str) -> str:
    return prompt.replace("PLACEHOLDER", " and ".join(subject_descriptions))


def _token_ids_without_eos(text: str) -> List[int]:
    token_ids = tokenizer.encode(text, return_tensors="pt")[0].tolist()
    if token_ids:
        return token_ids[:-1]
    return token_ids


def _candidate_call_phrases(subject: str) -> List[str]:
    phrases: List[str] = []

    def add_phrase(phrase: str) -> None:
        phrase = str(phrase or "").strip()
        if phrase and phrase.lower() not in {existing.lower() for existing in phrases}:
            phrases.append(phrase)

    add_phrase(subject)
    canonical = normalize_object_label(subject)
    add_phrase(canonical)
    for alias in V4_OBJECT_ALIASES.get(canonical, []):
        add_phrase(alias)
    if subject != canonical:
        subject_words = str(subject).split()
        if subject_words:
            add_phrase(subject_words[-1])
    return phrases


def _find_subsequence_locations(sequence: List[int], subsequence: List[int]) -> List[List[int]]:
    if not sequence or not subsequence or len(subsequence) > len(sequence):
        return []
    locations = []
    for start_idx in range(0, len(sequence) - len(subsequence) + 1):
        if sequence[start_idx:start_idx + len(subsequence)] == subsequence:
            locations.append(list(range(start_idx, start_idx + len(subsequence))))
    return locations


def get_call_ids_from_natural_prompt_flux(prompt: str, subjects: List[str], debug: bool = False) -> Optional[List[List[int]]]:
    prompt_token_ids = _token_ids_without_eos(prompt)
    prompt_token_strs = tokenizer.batch_decode(prompt_token_ids)
    call_ids: List[List[int]] = []
    missing_subjects = []

    for subject in subjects:
        subject_call_ids: List[int] = []
        for phrase in _candidate_call_phrases(subject):
            phrase_ids = _token_ids_without_eos(phrase)
            matches = _find_subsequence_locations(prompt_token_ids, phrase_ids)
            for match in matches:
                for token_idx in match:
                    if token_idx not in subject_call_ids:
                        subject_call_ids.append(token_idx)
        if not subject_call_ids:
            missing_subjects.append(subject)
        call_ids.append(sorted(subject_call_ids))

    if missing_subjects:
        print(
            "WARNING: natural-language prompt did not mention these pkl cube names; "
            f"falling back to PLACEHOLDER-style prompt binding: {missing_subjects}"
        )
        return None

    if debug:
        for subject, subject_call_ids in zip(subjects, call_ids):
            token_strs = [prompt_token_strs[token_idx] for token_idx in subject_call_ids]
            print(f"natural prompt call ids for {subject}: {subject_call_ids} -> {token_strs}")
    return call_ids


def build_prompt_and_call_ids(subject_descriptions: List[str], prompt: str, subject_embeds: List[Dict[str, Any]]) -> Tuple[str, str, List[List[int]]]:
    if "PLACEHOLDER" in prompt:
        placeholder_token_prompt = build_placeholder_token_prompt(subject_descriptions, prompt)
        ...
        return prompt, build_filled_prompt(subject_descriptions, prompt), call_ids

    call_ids = get_call_ids_from_natural_prompt_flux(
        prompt=prompt,
        subjects=subject_descriptions,
        debug=False,
    )
    if call_ids is not None:
        return prompt, prompt, call_ids

    fallback_prompt = prompt.rstrip() + " Objects from the scene file: PLACEHOLDER."
    fallback_token_prompt = build_placeholder_token_prompt(subject_descriptions, fallback_prompt)
    call_ids = get_call_ids_from_placeholder_prompt_flux(
    ...
    return fallback_prompt, build_filled_prompt(subject_descriptions, fallback_prompt), call_ids
...
def process_scene(
    ...
    placeholder_prompt, flux_prompt, call_ids = build_prompt_and_call_ids(
        subject_descriptions,
        placeholder_prompt,
        subject_embeds,
    )
    print("实际送入flux的prompt:", flux_prompt)
    print("送入flux的call_ids:", call_ids)
    ...
    copied_sidecars = copy_scene_sidecar_files(pkl_path, root_save_dir)
    if copied_sidecars:
        print("Copied scene sidecar files:", copied_sidecars)
    else:
        print("No same-name scene .pkl/.json/.jpg files found next to pkl.")
...
def visualize_input_conditions(pkl_path, placeholder_prompt):
    ...
    # ── 2. Build the prompt preview ──────────────────────────────────────────
    preview_prompt = build_v4_prompt(subject_descriptions, placeholder_prompt)
    if "PLACEHOLDER" in preview_prompt:
        full_prompt = build_placeholder_token_prompt(subject_descriptions, preview_prompt)
        display_prompt = build_filled_prompt(subject_descriptions, preview_prompt)
    else:
        full_prompt = preview_prompt
        display_prompt = preview_prompt
