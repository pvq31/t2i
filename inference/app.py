...
def generate_image_event(camera_elevation, camera_lens, surrounding_prompt, checkpoint_name, 
    ...
    # Create JSONL
    # Blender 输出会被保存成 cv_render.jpg 和多张 segmask，同时写入 cuboids.jsonl
    jsonl = [{
...
class SceneManager:
    ...
    def _preferred_prompt_description(self, asset_type):
        asset_type = str(asset_type or "").strip()
        if not asset_type or asset_type == "Custom":
            return "object"
        return TYPE_PROMPT_DESCRIPTIONS.get(asset_type, asset_type)


    def _align_description_with_type(self, description, asset_type):
        raw_description = str(description or "").strip()
        asset_type = str(asset_type or "").strip()

        if not asset_type or asset_type == "Custom":
            return raw_description or "object"

        preferred_description = self._preferred_prompt_description(asset_type)
        cleaned_description = re.sub(r"\s+\d+$", "", raw_description).strip()
        normalized_description = normalize_asset_text(cleaned_description)
        normalized_type = normalize_asset_text(asset_type)
        normalized_preferred = normalize_asset_text(preferred_description)

        if not normalized_description or normalized_description in GENERIC_DESCRIPTION_TOKENS:
            return preferred_description

        if normalized_type in normalized_description or normalized_preferred in normalized_description:
            return cleaned_description

        return preferred_description


    def get_subject_descriptions_for_prompt(self):
        return [
            self._align_description_with_type(obj.get('description'), obj.get('type'))
    ...
    def update_cuboid_description(self, obj_id, new_description):
        ...
        if 0 <= obj_id < len(self.objects):
            if new_description.strip():  # Check not empty
                self.objects[obj_id]['description'] = self._align_description_with_type(
                    new_description.strip(),
                    self.objects[obj_id].get('type')
                )
                return True
        return False
    ...
    def save_scene_to_pkl(self, filepath=None):
        ...
        subjects_data = []
        for obj in self.objects:# 写入pkl的代码
            subject_dict = {
                'name': self._align_description_with_type(obj['description'], obj['type']),
    ...
    def load_scene_from_pkl(self, filepath):
        ...
        try:
            ...
            for subject_dict in subjects_data:
                ...
                cuboid = {
                    'id': obj_id,
                    'description': self._align_description_with_type(name, asset_type),
    ...
    def _load_asset_dimensions(self):
        ...
        assets_dir = osp.dirname(osp.abspath(__file__))
        pkl_path = osp.join(assets_dir, "asset_dimensions.pkl")
        json_path = osp.join(assets_dir, "asset_dimensions.json")
        ...
        if osp.exists(pkl_path):
            try:
                ...
                print(f"Warning: Could not load asset dimensions from {pkl_path}: {e}")

        if osp.exists(json_path):
            try:
                with open(json_path, 'r', encoding='utf-8') as f:
                    raw_dimensions = json.load(f)
                return {
                    key: [float(value[0]), float(value[1]), float(value[2])]
                    for key, value in raw_dimensions.items()
                }
            except Exception as e:
                print(f"Warning: Could not load asset dimensions from {json_path}: {e}")

        print(f"Warning: asset dimensions not found in {assets_dir}")
        return {}
    ...
    def add_cuboid(self, description="New Cuboid", asset_type="Custom"):
        ...
        cuboid = {
            'id': obj_id,
            'description': self._align_description_with_type(description, asset_type),
    ...
    def _convert_to_blender_format(self): # 结构化layot控制
        """Convert internal objects format to server expected format"""
        subjects_data = []
        
        for obj in self.objects:
            subject_data = {
                'subject_name': self._align_description_with_type(obj['description'], obj['type']),
